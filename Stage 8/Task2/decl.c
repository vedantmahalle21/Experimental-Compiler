...
void makeAdrrCopy(int ci_1, int ci_2){
    int adrs_1 = 4097 + ci_1*8;
    int cou_ = 0;
    while(cou_ != 7){
        adro[ci_1][cou_] = adro[ci_2][cou_];
        if(fn_lo[ci_2][cou_] != -1){
            fn_lo[ci_1][cou_] = adrs_1+cou_;
        }
        else{
            fn_lo[ci_1][cou_] = fn_lo[ci_2][cou_];
        }
        cou_++;
    }
...
struct ASTNode* makeOperatorNode(char* op, struct ASTNode *l, struct ASTNode *r, int type)
{
    ...
    /*else if(l->nodetype == 25 && r->nodetype == 25){
        if(l->Ctype != r->Ctype){
            int r =0;
            struct ClassTable* temp1;
            temp1 = r->Ctype->Parentptr;
            while(temp1 != NULL){
                if(temp1 == l->Ctype)
                {
                    r = 1;
...
{
    int i,j,q,r,s,s1,p,t1,l1,l2,p1,p3,q2;
    char* c, strg;
    int flag = 0;
    if(t->type == 13){
        //printf("type.%d\n", t->type);
        ...
        if(kl == 0){
            adr = adr+2; fprintf(targetfile, "MOV BP, SP\n");
            p = getReg();
            struct Lsymbol *temp = t->Lentry;
            int u = 1;
            while(temp != NULL){
                adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
                temp->address = u;
                u++;
                temp = temp->next;
            }
            ...
            while(doi != 0){
                ...
                while(fni != 7){
                    if(adro[doj][fni] != -1){
                        adr = adr+2; fprintf(targetfile, "MOV R%d, %d\n", p, adro[doj][fni]);
                        adr = adr+2; fprintf(targetfile, "MOV [%d], R%d\n", fn_lo[doj][fni], p);
                    }
                    fni++;
                }
                doi--;
            ...
            adr = adr+2; fprintf(targetfile, "PUSH BP\n");
            adr = adr+2; fprintf(targetfile, "MOV BP, SP\n");
            p = getReg();
            struct Lsymbol *temp = t->Lentry;
            int u = 1;
            ...
            if(t->Gentry != NULL){
                ...
                p = getReg();
                int cl_i = t->Ctype->Class_index;
                int fn_i = mem_temp->Funcposition;
                int addrs = 4097 + cl_i*8;
                addrs = addrs + fn_i;
                adro[cl_i][fn_i] = adrc;
                fn_lo[cl_i][fn_i] = addrs;
                //printf("%d\n", adrc);
                freeReg();
            }
            while(temp != NULL){
            ...
            //printf("Func Ret\n");
            while(u > 1){
                adr = adr+2; fprintf(targetfile, "POP R%d\n", q);
                u--;
            }
            freeReg();
            j = CodeGen(t->right, targetfile);
            p = getReg();
            adr = adr+2; fprintf(targetfile, "MOV R%d, BP\n", p);
            adr = adr+2; fprintf(targetfile, "SUB R%d, 2\n", p);
            adr = adr+2; fprintf(targetfile, "MOV [R%d], R%d\n", p, j);
            freeReg();
            freeReg();
            adr = adr+2; fprintf(targetfile, "MOV BP, [BP]\n");
            q = getReg();
            adr = adr+2; fprintf(targetfile, "POP R%d\n", q);
            freeReg();
            adr = adr+2; fprintf(targetfile, "RET\n");
            //printf("%s Completed\n", t->varname);
        }
    }
    else if(t->type == 14){
        ...
        if(temp4 != NULL){
            ...
            if(ma == 1){
                ...
                adr = adr+2; fprintf(targetfile, "MOV R%d, BP\n", p);
                adr = adr+2; fprintf(targetfile, "SUB R%d, %d\n", p, 3);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", p, p);
                adr = adr+2; fprintf(targetfile, "ADD R%d, %d\n", p, temp->fieldIndex+1);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", p, p);
                adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        ...
        if(temp4 != NULL){
            if(ma == 1){
                ...
                int f_a = temp->Funcposition;
                p = getReg();
                adr = adr+2; fprintf(targetfile, "MOV R%d, %d\n", p, s+1);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", p, p);
                adr = adr+2; fprintf(targetfile, "ADD R%d, %d\n", p, f_a);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n",p, p);
                adr = adr+2; fprintf(targetfile, "CALL R%d\n", p);
                freeReg();
            }
            else if(r4 == 1){
                r4 = 0;
                p = getReg();
                struct MemberFuncList *temp1 = LookupMemFunc(t->left->varname, temp5);
                adr = adr+2; fprintf(targetfile, "SUB R%d, %d\n", p, 3);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", p, p);
                struct FieldList *temp = LookupField(t->right->varname, head5);
                adr = adr+2; fprintf(targetfile, "ADD R%d, %d\n", p, temp->fieldIndex+1);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", p, p);
                int f_a = temp1->Funcposition;
                adr = adr+2; fprintf(targetfile, "ADD R%d, %d\n", p, f_a);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n",p, p);
                adr = adr+2; fprintf(targetfile, "CALL R%d\n", p);
                freeReg();
            ...
            adr = adr+2; fprintf(targetfile, "CALL Y%d\n", t->left->Gentry->flabel);
        }
        adr = adr+2; fprintf(targetfile, "POP R%d\n", g+1);
        q = getReg();
        while(no_temp1 != 0){
        ...
        freeReg();
        while(g != -1){
            //adr = adr+2; fprintf(targetfile, "%d\n", g);
            q = getReg();
            adr = adr+2; fprintf(targetfile, "POP R%d\n", q);
            g--;
        }
        ...
        adr = adr+2; fprintf(targetfile, "MOV R%d, \"Alloc\"\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile,"CALL 0\n");
        adr = adr+2; fprintf(targetfile, "POP R%d\n", p);
        adr = adr+2; fprintf(targetfile, "POP R%d\n", q);
        adr = adr+2; fprintf(targetfile, "POP R%d\n", q);
        adr = adr+2; fprintf(targetfile, "POP R%d\n", q);
        ...
        p2 = getReg();
        int addrs = 4097 + (t->Ctype->Class_index)*8;
        adr = adr+2; fprintf(targetfile, "MOV R%d, %d\n", p2, addrs);
        if(temp != NULL){
            adr = adr+2; fprintf(targetfile, "MOV [%d], R%d\n", s, p);
            adr = adr+2; fprintf(targetfile, "MOV [%d], R%d\n",  s+1, p2);
            ...
            adr = adr+2; fprintf(targetfile, "MOV [R%d], R%d\n", p1 , p);
            adr = adr+2; fprintf(targetfile, "ADD R%d, 1\n", p1);
            adr = adr+2; fprintf(targetfile, "MOV [R%d], R%d\n", p1 , p2);
            freeReg();
            freeReg();
        }
        ...
        p = getReg();
        adr = adr+2; fprintf(targetfile, "MOV R%d, \"Heapset\"\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        ...
        if(t->left->type != 23){
            ...
            struct FieldList *temp2 = LookupField(t->left->varname, head5);
            adr = adr+2; fprintf(targetfile, "MOV R%d, BP\n", p1);
            adr = adr+2; fprintf(targetfile, "SUB R%d, %d\n", p1, 3);
            adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", p1, p1);
            adr = adr+2; fprintf(targetfile, "ADD R%d, %d\n", p1, temp2->fieldIndex);
        ...
        p = getReg();
        adr = adr+2; fprintf(targetfile, "MOV R%d, \"Free\"\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p1);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", p);
        ...
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", q);
        adr = adr+2; fprintf(targetfile, "PUSH R%d\n", q);
        adr = adr+2; fprintf(targetfile, "INT %d\n", 6);
        adr = adr+2; fprintf(targetfile, "POP R%d\n", p);
        adr = adr+2; fprintf(targetfile, "POP R%d\n", p);
        adr = adr+2; fprintf(targetfile, "POP R%d\n", p);
        adr = adr+2; fprintf(targetfile, "POP R%d\n", p);
        ...
        if(t->left->op == NULL){
            ...
            if(t->left->type == 5){
                ...
                adr = adr+2; fprintf(targetfile, "MOV R%d, %s\n", q, t->left->varname);
            }
            else if(t->left->type == 12){
                s = CodeGen(t->left, targetfile);
                adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", q, s);
        ...
        if(t->right->type == 3){
            ...
            q = getReg();
            adr = adr+2; fprintf(targetfile, "MOV R%d, \"NULL\"\n", q);
        }
        else if(t->right->type == 25){
            struct Gsymbol *temp = Lookup(t->right->varname);
            s = temp->address;
            q = getReg();
            q2 = getReg();
        ...
        switch(*(t->op)){
            case '+' : adr = adr+2; fprintf(targetfile, "ADD R%d, R%d\n", p, q);
                       freeReg();
                       return p;
                       break;
            case '-' : adr = adr+2; fprintf(targetfile, "SUB R%d, R%d\n", p, q);
                       freeReg();
                       return p;
                       break;
            case '*' : adr = adr+2; fprintf(targetfile, "MUL R%d, R%d\n", p, q);
                       freeReg();
                       return p;
                       break;
            case '/' : adr = adr+2; fprintf(targetfile, "DIV R%d, R%d\n", p, q);
                       freeReg();
                       return p;
                       break;
            case '%' : adr = adr+2; fprintf(targetfile, "MOD R%d, R%d\n", p, q);
                       freeReg();
                       return p;
                       break;
        ...
        else if(t->left->type == 19)
        {
            ...
            adr = adr+2; fprintf(targetfile, "MOV R%d, BP\n", p);
            adr = adr+2; fprintf(targetfile, "SUB R%d, 3\n", p);
            adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n", p, p);
            adr = adr+2; fprintf(targetfile, "ADD R%d, %d\n", p, temp->fieldIndex);
            adr = adr+2; fprintf(targetfile, "MOV R%d, [R%d]\n",p, p);
        }
        ...
        if(*(t->op) == '>'){    adr = adr+2; fprintf(targetfile, "GT R%d, R%d\n", p, q);
                                freeReg();
                                return p;
                            }
        else if(*(t->op) == '<'){       adr = adr+2; fprintf(targetfile, "LT R%d, R%d\n", p, q);
                                        freeReg();
                                        return p;
                            }
        else if(*(t->op) == 'E')      {adr = adr+2; fprintf(targetfile, "EQ R%d, R%d\n", p, q);
                            freeReg();
                            return p;
                            }
        else if(*(t->op) == 'N')      {adr = adr+2; fprintf(targetfile, "NE R%d, R%d\n", p, q);
                            freeReg();
                            return p;
                            }
            else if(*(t->op) == 'G')      {adr = adr+2; fprintf(targetfile, "GE R%d, R%d\n", p, q);
                            freeReg();
                            return p;
                            }
            else if(*(t->op) == 'L')      {adr = adr+2; fprintf(targetfile, "LE R%d, R%d\n", p, q);
                            freeReg();
                            return p;
                            }
            else if(*(t->op) == 'A')        {   i = getReg();
                                                adr = adr+2; fprintf(targetfile, "MOV R%d, 1\n", i);
                                                adr = adr+2; fprintf(targetfile, "EQ R%d, R%d\n", p, i);
                                                l1 = l;
                                                l++;
                                                adr = adr+2; fprintf(targetfile, "JZ R%d, X%d\n", p, l1);
                                                adr = adr+2; fprintf(targetfile, "MOV R%d, 1\n", i);
                                                adr = adr+2; fprintf(targetfile, "EQ R%d, R%d\n", q, i);
                                                freeReg();
                                                adr = adr+2; fprintf(targetfile, "JZ R%d, X%d\n", q, l1);
                                                ...
                                                l2 = l;
                                                l++;
                                                adr = adr+2; fprintf(targetfile, "JMP X%d\n", l2);
                                                adr = adr+2; fprintf(targetfile, "X%d:\n", l1);
                                                adr = adr+2; fprintf(targetfile, "MOV R%d, 0\n", p);
                                                adr = adr+2; fprintf(targetfile, "X%d:\n", l2);
                                                freeReg();
                                                return p;
                                            }
