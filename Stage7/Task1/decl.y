...
%type <n2> Program FDefBlock MainBlock ID FDefList Slist Stmt Retstmt InputStmt OutputStmt AsgStmt Ifstmt Whilestmt BreakStmt ContinueStmt AllocStmt FreeStmt expr var FIELD string MAIN E Type NUL InitializeStmt STRING MethodDefns
...
GDeclBlock  : DECL GDeclList ENDDECL    {   $$ =$2;Display($2); printf("Global created\n");}
            | DECL ENDDECL                                {}
            ;
...
FIELD   : ID '.' ID                       {$$ = $3;$$->left = $1;$1->right = $$;}
        ...
        | SELF '.' ID                 {$$ = $3;}
        | SELF '.' ID '(' ArgList ')'  {$$ = makeFuncCallNode($3, $5, 15);  makeANull();}
        | ID '.' ID '(' ArgList ')'         { struct MemberFuncList *temp = LookupM($3->varname, $1->Ctype);
                                              check(temp->paramlist, $5);
                                              $$ = makeFuncCallNodeC($3, $1, $5, 15);makeANull();}
        | ID '.' ID '(' ')'                 {$$ = makeFuncCallNodeC($3, $1, NULL, 15);}
        ; 
